...
class SellController extends Controller
{
    ...
    public function sellItem(SellRequest $request)
    {
        ...
        $item = new Item(); //Itemモデルのインスタンス化

        //ここから下は左がカラム名、右側がRequestでbladeのname属性から値を取得
        // つまり、左のカラムに対して、右側のRequestが入力した値を代入
        $item->image_file_name = $imageName;
        $item->seller_id = $user->id;

        $item->name = $request->input('name');
        // $item->name = $request->name; //この書き方でも可
